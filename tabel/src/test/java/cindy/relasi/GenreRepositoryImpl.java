...
@Override
@Transactional
public void deletedById(@NotNull Long id){
...
public List<Genre> findAll(@NotNull SortingAndOrderArguments args){
  ...
  VALID_PROPERTY_NAMES.contains(args.getSort().get())
    ...
    qlString += " ORDER BY g." + args.getSort().get() + "" +
    args.getOrder().get().toLowerCase();

  }

TypedQuery<Genre> query = entityManager.createQuery(qlString, Genre.class);
query.setMaxResult(args.getMax().orElseGet(applicationConfiguration::getMax ));

args.getOffset().ifPresent(query::setFisrtResult);
