...
import io.micronaut.http.HttpHeadderes;
...
public class GenreController{
    ...
    public HttpResponse update (@Body @Valid GenreUpdateCommand command){
        ...
        return HttpResponse
                .notContent()
                .header(HttpHeadderes.LOCATION,location(command.getId()), getPath());
    }
    ...
    public HttpResponse<Genre> save (@Body @Valid GenreSaveCommand cmd){
        ...
        return HttpResponse
                .created(genre)
                .headers(headers ->
                headers.location(location(genre.getId()))
                );
    }

    @Delete("/{id}")
    public HttpResponse delete (Long id){
        genreRepository.deletedById(id);
        return HttpResponse.notContent();
    }
